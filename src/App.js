...
const App = () => {
  ...
  if (!isLoggedIn) return <LoginSection onLogin={() => setIsLoggedIn(true)} />;
    ...
    <div style={styles.appContainer}>
      <style>{`
        .excel-table { width: 100%; border-collapse: collapse; table-layout: fixed; border: 2.2px solid black; }
        ...
        <div style={styles.scrollWrapper}>
          <div style={styles.invoiceOuter}>
            <div ref={invoiceRef} style={styles.a4Sheet}>
              ...
              <div style={styles.sigSection}><div style={styles.sigBox}><div style={styles.sigName}>Zwe</div><div style={styles.sigLine}>Zwe Htet Naing</div></div></div>
            </div>
          </div>
...
const LoginSection = ({ onLogin }) => (
  <div style={styles.loginBg}><div style={styles.loginCard}><h3>OASIS LOGIN</h3><input id="u" placeholder="User" style={styles.loginInput}/><input id="p" type="password" placeholder="Pass" style={styles.loginInput}/><button onClick={() => onLogin()} style={styles.saveBtn}>Login</button></div></div>
);
...
const styles = {
  ...
  a4Sheet: { width: '210mm', minHeight: '297mm', padding: '15mm', backgroundColor: 'white', boxShadow: '0 10px 25px rgba(0,0,0,0.1)' },
  ...
  bizSub: { color: '#10b981' },
  serviceText: { fontSize: '11px', margin: 0, color: '#666', maxWidth:'350px' },
  invoiceBadge: { background: '#10b981', color: 'white', padding: '10px 40px', fontWeight: 'bold', fontSize:'20px' },
  ...
  signatureSection: { marginTop: '50px', display: 'flex', justifyContent: 'flex-end' },
